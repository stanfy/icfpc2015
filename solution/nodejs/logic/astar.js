...
(function (definition) {
    ...
    var sf = function (n) {
        var path = pathTo(n);
        var res = [];
        path.reverse().forEach(function (x) {
            res.push("" + f(x) + "->");
        });
        return res.join(" ");
    }
    ...
    var astar = {
         ...
         *          astar.heuristics).
         */
        search: function (graph, startState, endState, options, endsStates) {
            start = new GridNode(startState.state.unit.pivot.x, startState.state.unit.pivot.y, 0, 0, 1);
            this.cleanNode(start);
            start.state = startState;
            start.step = "initial";

            ends = endsStates.map(function (es) {
                var gridNode = new GridNode(es.state.unit.pivot.x, es.state.unit.pivot.y, 0, 0, 1);
                this.cleanNode(gridNode);
                gridNode.state = es;
                return gridNode;
            }.bind(this));
            ...
            while (openHeap.size() > 0 && openHeap.size() < 10000 && this.visitedCount < 5000) {

                var currentNode = openHeap.pop();
                var me = brain.unitsAreEqual(currentNode.state.state.unit, ends[0].state.state.unit);
                if (me) {
                ...
                ends.forEach(function (end) {
                    if (brain.unitsAreEqual(currentNode.state.state.unit, end.state.state.unit)) {
                        var endHash = end.state.state.unit;
                        foundPaths[endHash] = currentNode;
                    }
                });
            ...
            for (var i = 0; i < ends.length; i++) {
                var end = ends[i];
                var endHash = end.state.state.unit;
                if (foundPaths[endHash]) {
                    return pathTo(foundPaths[endHash]);
                }
            }
